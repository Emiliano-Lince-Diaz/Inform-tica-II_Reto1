    ...
    for (int f = 0; f < filas; f++) {
        for (int c = 0; c < columnas; c++) {
            ...
            if (leerFicha(tablero, indiceByte, offset) == 0) {
                ...
                // Esto es lo que permite que caigan fichas haciendo combo por suerte (cascadas)
...
void funcion_cascadas(unsigned char* tablero, int* estadoJuego) {
    ...
    do {
        ...
        if (hayCombinaciones) {
            ...
            cout << "\n--- Resolviendo Cascada #" << iteracionCascada << " ---" << endl;

            funcion_eliminacion_por_combinacion(tablero, mascaraFinal, estadoJuego);
            estadoJuego[6]++; // Aumentar CASCADAS totales

            // Imprimir el estado para que el usuario vea cómo caen
            imprimirTableroFichas(tablero, filas, columnas);
        }
